...
enum status_code Insert(struct Employee_Record_System Employee_Record1[], char employee_name[], char project_name[], unsigned int number_of_hours, char employee_address[], unsigned int phone_number, unsigned int employee_salary, int size)
{
	int i;
...
int getMaxNumHrs(struct Employee_Record_System Employee_Record1[], char project_name[], int size)
{
...
int isUnique(struct Employee_Record_System Employee_Record1[], int size)
{
	int i, j, n=-1;//here n counts the number of elements in final list.

	SortRecord(Employee_Record1, size);
...
int main(int argc, char const *argv[])
{
	...
	for (i = 0; i < n1; ++i)
	{
		...
		printf("Phone Number: ");
		scanf("%u", &Employee_Record1[i].phone_number);
		getchar();
...
11.To get the symmentric difference between the two lists.\n");
	...
	switch(opertn)
	{
		...
		case 11:
		{
			...
			for (i = 0; i < n2; ++i)
			{
				...
				printf("Phone Number: ");
				scanf("%u", &Employee_Record2[i].phone_number);
				getchar();
				printf("Employee Salary: ");
	...
	switch(opertn)
	{
		...
		case 6:
		{
			...
			gets(project_name);
			getch();
			max_hours=getMaxNumHrs(Employee_Record1, project_name, size);
