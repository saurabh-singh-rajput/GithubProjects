...
struct Employee_Record_System
{
	...
	unsigned long long int phone_number; //The phone number can be 10 digits long.
	...
	for (i = 0; i < n1 && !status; ++i)
	{
		if(strcmp(Employee_Record1[i].employee_name,employee_name)==0) //checks if name is same or not
			if(strcmp(Employee_Record1[i].project_name,project_name)==0) //if name is same then only checks project name
			{
				...
				status=UPDATE; //confirms the updation
			}
	}
...
void getNumRecords(struct Employee_Record_System Employee_Record1[], int n1)
{
	int count=0, i;
...
{
	int i, j, n=0;//here n counts the number of elements in final list.
...
{
	int i=0, j=0, k=0, flag;
	struct Employee_Record_System temp1[size], temp2[size];
	int n3, n4, n5;	

	Initialize(Employee_Record3, 2*size);

	n4=list_Union(Employee_Record1, Employee_Record2, n1, n2, size, temp1);
	n5=list_intersection(Employee_Record1, Employee_Record2, n1, n2, size, temp2);
	n3=list_difference(temp1, temp2, n4, n5, size, Employee_Record3);
	...
	while(opertn!=12)
	{
		...
		switch(opertn)
		{
			...
			case 1:
			{
				...
				if(status==2)
				{
					//We don't increament the size if list is updated.
					printf("\nInsertion was successful.\n");
				}
				else if(status)
				{
					printf("\nInsertion is successful.\n");
					n1++; //we increament the size by one as record is added at new place.
				}
				else
				{
					printf("\nInsertion failed. The list is full.\n");
				}
