...
void InsertInDataNode(Btree *root, int value){
	int i=root->u.DNode.numValues-1;

	while(value<root->u.DNode.values[i] && i>=0){
		root->u.DNode.values[i+1]=root->u.DNode.values[i];
		i--;
	}

	/*UPDATE HERE!!!*/
	root->u.DNode.values[i+1]=value;
...
void SplitLeaf(int medentry, Btree *medright, Btree *root, int pos, int *newmedian, Btree **newright){
	int i,median;
	if(pos<=DMIN-1)
	{
		median=DMIN-1;
	...
	*newmedian=(*newright)->u.DNode.values[0];
	printf("newmedian:%d\n", *newmedian);
	//printf("ok2\n");
...
int PushDown(int value, Btree *root, int *medNum, Btree **rightSub){
	int pos;
	//printf("adsadada\n");
	if(root->flag){
		int i=0;
		while(i<root->u.DNode.numValues && value>root->u.DNode.values[i]){
			i++;
		}

		if(root->u.DNode.numValues<DMAX/*Maximum values in DNode*/){
		//printf("dkajsdanksndnd\n");
		...
		//printf("pos:%d",root->u.KeyNode.branch[pos]->u.DNode.values[0]);
...
Btree * Insert(Btree *root, int value){
	...
	else if(PushDown(value, root, &medNum, &rightSub)){
...
void MoveLeftLeaf(Btree *root,int pos)
{
	...
	root->u.KeyNode.keys[pos]=rhs->u.DNode.values[1];
...
void MoveRightLeaf(Btree *root,int pos)
{
	...
	root->u.KeyNode.keys[pos]=rhs->u.DNode.values[lhs->u.DNode.numValues-1];	//should be rhs->u.data.arr[0].account_no
	lhs->u.DNode.numValues--;	
}
...
void DeleteFromLeafDirect(int target,Btree *root)
{
	...
	if(root->flag)
	{
		while(root->u.DNode.values[i]<target)
		{
			i++;
		}
		if(root->u.DNode.values[i]>target)
		{
			printf("ENTRY TO BE DELETED IS ABSENT\n");
		}
		else if(root->u.DNode.values[i]==target)
		{
			i++;
...
Btree * Delete(int target,Btree *root)
{
	...
	//	printf("in delete- flag k\n");
		Delete_rec(target,root);						//returns some value
...
void RangeSearch(Btree *root, int start, int end){
	...
	for(i=0; i<ptr->u.DNode.numValues; i++){
		if(ptr->u.DNode.values[i]==start)
			break;
	}

	while(ptr!=NULL && ptr->u.DNode.values[i]<=end ){
		printf("%d\t", ptr->u.DNode.values[i]);
		i++;
