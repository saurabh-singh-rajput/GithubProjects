...
#define ORDER 5 
#define KMIN 2 //minimum number of keys
#define KMAX 4 //maxmimum number of keys
#define DMIN 2 //minimum number of values
#define DMAX 4 //maxmimum number of values

typedef enum {FALSE, TRUE} bool;

//Keeps count of the records inserted.
int count=0;
...
typedef struct BPlusTree{
	union{
		struct KeyNodes
		{
			int keys[ORDER]; //array of keys.
			struct BPlusTree *branch[ORDER]; //array of branch pointers.
			int numKeys; //total number of keys.
		}KeyNode;
		
		//Structure for doubly linked list.
		struct DNodes{
			Record values[DMAX+1]; //Array of records in single node.
			int numValues; //Number of vallues present in the node.
			struct BPlusTree *next, *prev;
		}DNode;
	}u;
	int flag; //to know which one to use - doubly linked list or key structure.
}Btree;

//Global pointer to point to the start of leaf node i.e. doubly linked list.
...
//Searches the node in which the target might be present.
...
//checks if the record is present in the node.
...
//helper function for InsertInDataNode. Checks if the node already exists.
...
//Inserts record in appropriate position.
...
//Inserts key at appropriate position.
...
//Splits key if the number of keys exceed the limit.
...
//Splits leaf if the number of values exceed the limit.
...
//Searches the next position wheree we have to go.
...
//Recursive function which traverses the entire key for inserttion and make changes appropriately in  the entire tree.
...
//moves key to the right sibling.
...
//moves record to the left sibling.
...
//moves record to the right sibling.
...
//Deletes record from the leaf node directly
...
//checks if the leaf needs to be merged and returns 1 if leaf is merged else 0
...
//checks if the key needs to be merged and returns 1 if key is merged else 0
...
//Recurseive helpere function to delete record.
int recDelete(int target,Btree *root){
	...
	if((root->u.KeyNode.branch[0])->flag){
		...
		searchPosition(target,root,&pos);
		if(recDelete(target,root->u.KeyNode.branch[pos])){
...
//Searches all the values present between start and end.
...
bool checkValidity(Record rec){
	...
	flag=0;
	for(i=0; rec.designation[i]!='\0' && !flag; i++){
