...
/* we assume that the memory is divided into 1000 memory locations each consisting on gran bytes. So minimum space allocated 
...
It divides the heap in two parts: one totally occupies and one totally free.*/
...
// following the FIRST FIT method;	
	...
	headAloc=Sort(headAloc); // to mintain a sorted allocate list
	*headFree=ptrFree;
	return headAloc;
}

// to coalesce the adjacent free chunks
...
// to free particuar allocated chunk.
    ...
    while(flag==1){
    	...
    	switch(n)
    	{
    		case 1:printf("1. insert an integer\n");
    			   ...
    			   printf("gran. insert an array of integers\n");
    		       ...
    		       switch(x)
    		       {
    		       	  ...
    		       	  case 4: printf("enter the name of the pointer");
    		       	  	      ...
    		       	  	      s=sizeof(double);
    		       	  	      break;
    		       	  case gran: printf("enter the name of the pointer");
    		       	  		  scanf("%s",stringname);
